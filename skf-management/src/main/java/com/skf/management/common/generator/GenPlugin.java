...
public class GenPlugin extends PluginAdapter {
	...
	public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
		...
		StringBuilder updateSQL = new StringBuilder("update ").append(tableName).append(" set ")
				.append(pkColumn.getActualColumnName()).append(" = #{item.").append(pkColumn.getJavaProperty())
				.append("}\n");
		...
		for (IntrospectedColumn introspectedColumn : introspectedTable.getAllColumns()) {
			...
			// 拼接IF语句
			ifSQL.append("      <if test=\"null != item.").append(javaProperty).append(" and '' != item.")
		...
		rootElement.addElement(
				createSql("sql_save_columns", saveColumn.append("\t) values").toString().replaceFirst(",", "")));
	...
	private XmlElement createSave(String id, IntrospectedColumn pkColumn) {
		...
		if (null != pkColumn) {
			...
			save.addElement(
					new TextElement("<include refid=\"sql_save_columns\" /><include refid=\"sql_save_values\" />"));
		} else {
